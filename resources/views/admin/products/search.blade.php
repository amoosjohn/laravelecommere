...
if(isset($row->categoryCommission)) {
    ...
    $commission = Functions::calculateCommission($categoryCommission,$price);
...
<tr>
    ...
    <td style="width: 8%;"><img src="{{ $image }}" class="img-responsive" style="width:100px;height:100px"/></td>
    ...
    <td style="width: 8%;text-align: center;">{{ ($row->sale_price==0)?$symbol.".".Functions::moneyFormat($row->price):$symbol.".".Functions::moneyFormat($row->sale_price) }}</td>
    <td style="width: 5%;text-align: center;">{{ $symbol.".".$commission }}</td>
    <td style="width: 10%;text-align: center;">{{ (count($row->users)>0)?$row->users->firstName:'' }}</td>
    <td style="width: 5%;text-align: center;"><span class="{{ $color }} sbold">{{ $status }}</span></td>
    <td style="width: 8%;">
        ...
        <a href="<?php echo url('admin/products/' . $row->id . '/edit'); ?>" class="btn btn-primary btn-sm" title="Edit"><i class="fa fa-edit"></i>
        </a>
        {!! Form::open(array('url' => 'admin/products/' .$row->id,'class'=>'form-inline form-delete','id'=>'delete')) !!}
